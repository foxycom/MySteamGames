...
$(ESPRESSO_TESTS) : app-androidTest.apk app-instrumented.apk
	...
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c com.joffreylagut.mysteamgames.mysteamgames))
	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear com.joffreylagut.mysteamgames.mysteamgames; \
	else \
